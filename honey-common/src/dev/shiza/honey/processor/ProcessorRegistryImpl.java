...
/**
* An implementation of the {@link ProcessorRegistry} interface which allows for keeping track
* and manipulating a list of preprocessors.
*/
final class ProcessorRegistryImpl implements ProcessorRegistry {
  ...
  */
  ...
  public ProcessorRegistry preprocessor(final Processor processor) {
    return new ProcessorRegistryImpl(ImmutableList.builder()
    .addAll(preprocessors)
    .add(processor)
    .build()
  }
  ...
  */
  ...
  public String preprocess(final String content) {
    return preprocessors.stream()
        .reduce(content, (processedContent, processor) -> processor.process(processedContent),
            (a, b) -> b);
  }
}
